...
import { Menu as MenuIcon, LayoutDashboard, ShieldCheck, BookOpen, Plus, Loader2 } from 'lucide-react';
import { useAirlockUpload } from '@/src/hooks/useAirlockUpload';
import clsx from 'clsx';

export function BottomTabs() {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { uploadFile, isUploading } = useAirlockUpload();
    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#FFFFFF] border-t border-[#E5E5E5] flex items-center justify-around z-50">
      ...
      <div className="relative -top-6">
        <button
          ...
          className={clsx(
            "flex items-center justify-center w-14 h-14 bg-[#111111] rounded-full text-white shadow-lg hover:scale-105 transition-transform",
            isUploading && "opacity-80 cursor-wait scale-100"
          )}
          aria-label="Upload"
          disabled={isUploading}
        >
          {isUploading ? <Loader2 size={28} className="animate-spin" /> : <Plus size={28} strokeWidth={2} />}
        </button>
      </div>
