...
BEGIN
    RAISE NOTICE 'Starting Governance Automation Verification...';

    -- 0. Schema Verification
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'governance_tasks') THEN
        RAISE EXCEPTION 'Schema Assertion Failed: governance_tasks table missing';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'governance_tasks' AND column_name = 'action_payload') THEN
        RAISE EXCEPTION 'Schema Assertion Failed: action_payload column missing';
    END IF;
    ...
    VALUES ('Test Entity Gov Automation', 'FAMILY')
    RETURNING id INTO v_entity_id;

    -- Create Asset
    INSERT INTO public.assets (name, entity_id, type, currency, owner_id)
    VALUES ('Test Asset Gov Automation', v_entity_id, 'BANK', 'USD', v_owner_id)
